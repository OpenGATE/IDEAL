    ...
    if (np.allclose(ref.GetOrigin(),target.GetOrigin())) and \
       ...
       (ref.GetLargestPossibleRegion().GetSize() == ref.GetLargestPossibleRegion().GetSize() ):
        if kwargs.get('verbose',False):
            print("Images with equal geometry, using the slightly faster implementation.")
        return gamma_index_3d_equal_geometry(ref,target,**kwargs)
    else:
        if kwargs.get('verbose',False):
            print("Images with different geometry, using the slightly slower implementation.")
...
def gamma_index_3d_equal_geometry(imgref,imgtarget,dta=3.,dd=3., ddpercent=True,threshold=0.,defvalue=-1.,verbose=False,threshold_percent=False):
    """
    ...
    * threshold_percent is a flag, True means that threshold is given in percent, False (default) means that the threshold is absolute.
    ...
    gimg=itk_image_from_array(g.swapaxes(0,2).astype(np.float32).copy())
...
# FIXME: should this function remain public or be made private (by prefixing it with an _underscore)?
def gamma_index_3d_unequal_geometry(imgref,imgtarget,dta=3.,dd=3.,ddpercent=True,threshold=0.,defvalue=-1.,verbose=False,threshold_percent=False):
    """
    ...
    * `threshold_percent` is a flag, True means that threshold is given in percent, False (default) means that the threshold is absolute.
    ...
    if nmask==0:
        ...
        dummy = itk_image_from_array((np.ones(atarget.shape)*defvalue).swapaxes(0,2).copy())
    ...
    if verbose:
        ...
        print("{} target voxels are in the intersection of target and reference image.".format(noverlap))
...
class Test_GammaIndex3dIdenticalMesh(unittest.TestCase):
    def test_identity(self):
        ...
        img1 = itk_image_from_array(a_rnd)
        img2 = itk_image_from_array(a_rnd)
    ...
    def test_scaling(self):
        ...
        a_rnd = np.random.uniform(0.,10.,(4,5,6))
        img1 = itk_image_from_array(a_rnd)
        img2 = itk_image_from_array(1.03*a_rnd)
        img_gamma = gamma_index_3d_equal_geometry(img1,img2,dd=3.,dta=2.0)
    ...
    def test_checkerboards(self):
        ...
        img_odd = itk_image_from_array(a_odd)
        img_even = itk_image_from_array(a_even)
    ...
    def test_large_image(self):
        ...
        #for N in [1,2,5,10,20,50,100]:
            tgen = datetime.now()
            img_ref = itk_image_from_array(np.ones((N,N,N),dtype=float))
            img_target = itk_image_from_array(np.random.normal(1.,0.02,(N,N,N)))
...
class Test_GammaIndex3dUnequalMesh(unittest.TestCase):
    def test_EqualMesh(self):
        ...
        for i in range(5):
            ...
            img_ref = itk_image_from_array(np.ones(nxyz,dtype=float).swapaxes(0,2).copy())
            img_ref.SetOrigin(oxyz)
            img_ref.SetSpacing(sxyz)
            img_target = itk_image_from_array(np.random.normal(1.,0.05,nxyz).swapaxes(0,2).copy())
    ...
    def test_Shift(self):
        ...
        for i in range(5):
            ...
            img_ref = itk_image_from_array(data.swapaxes(0,2).copy())
            img_ref.SetSpacing(sxyz)
            img_ref.SetOrigin(oxyz)
            img_target = itk_image_from_array(data.swapaxes(0,2).copy()) # same as ref
    ...
    def test_Gradient(self):
        ...
        for i in range(5):
            ...
            img_ref = itk_image_from_array(refDATA.swapaxes(0,2).copy())
            ...
            img_target = itk_image_from_array(targetDATA.swapaxes(0,2).copy())
